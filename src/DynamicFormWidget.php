...
use Symfony\Component\CssSelector\CssSelectorConverter; // Updated to use CssSelectorConverter
...
class DynamicFormWidget extends \yii\base\Widget
{
    ...
    protected function registerOptions($view)
    {
        ...
        $this->_hashVar = self::HASH_VAR_BASE_NAME . hash('crc32', $encOptions);
    ...
    private function removeItems($content)
    {
        ...
        // Import all nodes from the crawler to the DOMDocument
        $crawler->filter('body')->each(function (Crawler $node) use ($document, $root) {
            $root->appendChild($document->importNode($node->getNode(0), true));
        });

        // Convert CSS selector to XPath using CssSelectorConverter
        $cssSelectorConverter = new CssSelectorConverter();
        $xpathQuery = $cssSelectorConverter->toXPath($this->widgetItem);
        $domxpath = new \DOMXPath($document);
        $crawlerInverse = $domxpath->query($xpathQuery);
        ...
        preg_match('/<._root>(.*?)<\/_root>/is', $html, $matches);
        return $matches[1] ?? $content; // Fallback to original content if extraction fails
    }
}
